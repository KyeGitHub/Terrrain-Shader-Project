...
C3dglBitmap roadTex;
GLuint idTexRoad;
C3dglBitmap roadNormalTex;
GLuint idTexRoadNormal;
GLuint whiteTextureId;
C3dglBitmap skyboxTexture;
GLuint idTexSkybox;
...
bool init()
{
	...
	if (!skybox.load("models\\sphere.obj")) return false;


	//texture loading
	grass.Load("textures/grass1.jpg", GL_RGBA);
	if (!grass.GetBits()) return false;

	roadTex.Load("textures/road.jpg", GL_RGBA);
	if (!roadTex.GetBits()) return false;

	roadNormalTex.Load("textures/roadNormal.png", GL_RGBA);
	if (!roadNormalTex.GetBits()) return false;

	skyboxTexture.Load("textures/skyboxTexture.jpg", GL_RGBA);
	if (!skyboxTexture.GetBits()) return false;
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, roadTex.GetWidth(), roadTex.GetHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, roadTex.GetBits());
	glGenerateMipmap(GL_TEXTURE_2D);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, roadNormalTex.GetWidth(), roadNormalTex.GetHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, roadNormalTex.GetBits());


	glActiveTexture(GL_TEXTURE0);
	glGenTextures(1, &idTexSkybox);
	glBindTexture(GL_TEXTURE_2D, idTexSkybox);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, skyboxTexture.GetWidth(), skyboxTexture.GetHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, skyboxTexture.GetBits());


	//sending data to sampler
	...
	glClearColor(0.59, 0.68, 0.86, 1.0f);   // blue sky background
...
void render()
{
	...
	Program.SendUniform("lightAmbient.color", 1, 1, 1);

	// setup directional light
	Program.SendUniform("lightDir.on", 0);
	Program.SendUniform("lightDir.direction", 10.0, 0.1, 5.0);
	Program.SendUniform("lightDir.diffuse", 1.0, 1.0, 1.0);
	Program.SendUniform("lightDir.specularColour", 1.0, 1.0, 1.0);
	Program.SendUniform("lightDir.specularPower", 5.0);

	//setup fog
	Program.SendUniform("fogColour", 0.59, 0.68, 0.86);
	Program.SendUniform("fogDensity", 0.3);
	Program.SendUniform("fogStart", 0);
	Program.SendUniform("fogEnd", 10);
	...
	vec4 cameraPos = inverse(matrixView)[3];
	cameraPos.y -= 2;

	m = translate(matrixView, vec3(cameraPos));
	...
	Program.SendUniform("materialDiffuse", 1, 1, 1);	// grassy green
	Program.SendUniform("materialAmbient", 1, 1, 1);
	Program.SendUniform("shininess", 0.0);
	...
	Program.SendUniform("materialDiffuse", 1, 1, 1);
	Program.SendUniform("materialAmbient", 1, 1, 1);
